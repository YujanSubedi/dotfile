...
myLayout =mkToggle (NOBORDERS ?? NBFULL ?? EOT) (monadtall ||| monadthreecol ||| monadwide ||| qtilemax)
  where
     ...
     monadtall   = gaps [(U,37), (D,5), (L,5), (R,5)] $ spacing gap $ Tall nmaster delta ratio

     monadthreecol   = gaps [(U,37), (D,5), (L,5), (R,5)] $ spacing gap $ ThreeColMid nmaster delta tclratio

     monadwide   = gaps [(U,37), (D,5), (L,5), (R,5)] $ spacing gap $ Mirror (Tall nmaster delta ratio)

     qtilemax   = gaps [(U,37), (D,5), (L,5), (R,5)] $ spacing gap $ Full
